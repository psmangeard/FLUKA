...
#include "TAxis.h"
using namespace std;
//This is a C++ macro to be used with ROOT to plot the "good" events from the FLUKA simulation run, stored in the file "eventdata.txt". 
//ROOT plots then the event in bending plane. and the non-bending plane. 
//To use it, go on ROOT and type: .L graph_event.cpp and then type main()

int main() {
	FILE *fp;
	fp = fopen("./eventdata.txt","r");																															//data file outputted by read_event program
	if (fp) {
	int oldevent;																																				
	int numFile=0;
	...
	int numLines =0;
	char string[BUFSIZ];																																					//string that will hold each line of the file
	for(size_t i = 0; fgets(string, sizeof(string),fp); ++i) {																												//loop over all lines of file
			numLines++;
			printf("%s", string);
			char  *line_elements[4];																															//array that will store elements of each line
			int n=0;
			char  *ptr;
			ptr = strtok (string," ,");																															//parse line
 			while (ptr!= NULL) {
			line_elements[n++] = ptr;																															//store the token in array
  			ptr = strtok(NULL, ",");
			}
			int event, id; 
			float f1, f2;
			...
			if (i==0) {																																			
				...
				c1->cd(2);;
   				gr2->Draw("AP");
				c1->SaveAs(Form("./event_0%d.jpg", numFile));
				c1->SaveAs(Form("./event_0%d.root", numFile));																								//also save it as root file to later work on the event
				c1->Update();
				oldevent = event;
				counter1 = 0;
			 	counter2 = 0;
			...
			if (event == oldevent) {
				...
				cout << "Total number of lines is " << numLines << endl;																						//plot the last event of file
				gROOT-> SetStyle("Plain");
				TCanvas *c1 = new TCanvas("c", Form("Event %d", oldevent),900,900);
				c1->Divide(2,1);																																//divide canvas into two pads
				TGraph *gr1 = new TGraph(counter1, x, z1);
